...
@Command(
    ...
    description = [
        "Create a mops launcher with the project's runtime paths.",
        "",
        "Uses the nearest Gradle wrapper to inspect `mpsDefaults` of plugin `com.specificlanguages.mps` or `RunAntScript` tasks of plugin `de.itemis.mps.gradle.common`, then writes build/mops or build/mops.cmd.",
        ...
        "Partial discoveries print available values and exit with status 1; complete pairs create the launcher and exit with status 0.",
    ],
)
class CreateLauncherCommand(private val root: MopsCommand) : Callable<Int> {
    @Parameters(
    ...
    override fun call(): Int {
        ...
        if (!guess.usableMps || !guess.usableJava) {
            ...
            out.println("Launcher: ${guess.writeLauncher(windows)}")
        }
        out.flush()
