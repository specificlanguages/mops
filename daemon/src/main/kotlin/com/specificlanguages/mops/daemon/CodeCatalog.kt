...
object CodeCatalog {
    ...
    private val entries = listOf(
        ...
        Entry("SNode", "child", "SNode.child: NodeChild", "none", "Live indexed accessor. child[role] returns one SNode or null, and throws for multiple children. Assignment replaces the role with one node; null clears it. Reads require model access; writes require command access. Declared cardinality and constraints are not checked."),
        Entry("SNode", "children", "SNode.children: NodeChildren", "none", "Live indexed accessor. children[role] returns an immutable list of current children in order, empty for a missing role. Assign a list to replace the role, or [] to clear. New children must be detached; existing children in the role may be retained or reordered. Reads require model access; writes require command access. Unknown writes fail. Declared cardinality and constraints are not checked. Shadows native getChildren(); use getChildren(containmentLink) for native iteration."),
        Entry("SNode", "references", "SNode.references: NodeReferences", "none", "Live indexed accessor. references[role] returns SReference or null; use targetNode to resolve it. Assign an SNode, SNodeReference, or SReference; null clears it. Reads require model access; writes require command access. Unknown writes fail. Native constraints are not checked. Shadows native getReferences(); use getReference(referenceLink) for native access."),
        Entry("SNode", "properties", "SNode.properties: NodeProperties", "none", "Live indexed accessor: properties[name] reads with model access; properties[name] = value writes with command access. Includes inherited properties. Missing or unset reads return null; unknown writes fail. Assign null to clear. Writes use native setProperty without constraint checks. In Groovy this shadows native getProperties(); descriptors are available through node.concept.properties."),
